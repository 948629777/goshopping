<template>
	<div class="trade-container">
		...
		<div class="content">
			...
			>
				<span
					:class="{ username: true, selected: isChecked == index }"
					@click="curCheck(index)"
					>{{ item }}</span
				>
				<p>
	...
	methods: {
		getCar() {
			...
			this.$axios({
				...
				this.goodsList = res.data.data[0].cartInfoList.filter(
					(item) => {
		...
		gettradeNo() {
			...
			this.$axios({
				...
				this.goodsList.forEach((item) => {
					this.subForm.orderDetailList.push({
						id: null,
						orderId: null,
						skuId: item.skuId,
						skuName: item.skuName,
						imgUrl: item.imgUrl,
						orderPrice: item.skuPrice,
						skuNum: item.skuNum,
						hasStock: null,
					})
				})
				this.$axios({
					...
					if (res.data.code === 200) {
						...
						setTimeout(() => {
							this.$router.push('/pay/' + res.data.data)
						}, 1500)
					} else {
